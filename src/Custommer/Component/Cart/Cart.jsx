...
function Cart() {
  ...
  const total =
    cart && cart.items
      ? cart.items.reduce((sum, v) => sum + v.price * v.quantity, 0)
      : 0;
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      dc = dc.map((v) => ({
        ...v,
        on:
          total < v.minOrderValue ||
          v.userUsedIds.find((v) => v === getUserId())
            ? false
            : true,
      }));
      setDiscounts(dc);
  ...
  return (
    <div className="bg-gray-100 p-6 w-[80%] mx-auto">
          ...
          {cart &&
            ...
            cart.items.map((v, index) => (
              <CartItem
                ...
                onRemove={() =>
                  handleOnPlusOrMinus(v, Number(`-${v.quantity}`))
                }
              />
            ))}
        </div>
                ...
                {selectDiscount ? (
                  <span>
                    {selectDiscount.type === "FIXED_AMOUNT"
                      ...
                      : (selectDiscount.value / 100) * total}
                  </span>
                ) : (
                  "0"
                )}
            ...
            {discounts &&
              discounts.map((v, i) => (
                <div className="bg-blue-200 p-3 flex justify-between" key={i}>
                  <div>Mã: {v?.name}</div>
                  <div>
                    Giảm:{v?.value} {v?.type === "FIXED_AMOUNT" ? "d" : "%"}
                  </div>
                  <div>Đơn tối thiểu: {v?.minOrderValue}Đ</div>
                  <input
                    type="radio"
                    name="concac"
      ...
      <AuthModal
        ...
        handleClose={() => setShowRegister(false)}></AuthModal>
    </div>
  );
}
